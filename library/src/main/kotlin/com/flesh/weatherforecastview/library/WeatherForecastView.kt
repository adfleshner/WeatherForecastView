 ...
 * This is the actual view being displayed
 */
class ForecastView : RelativeLayout {
    ...
    fun setForecastData(weatherInterface: WeatherForecastInterface) {
        ...
        val list = weatherInterface.getHeatherHelperList()

        val isNight = list[0].startPeriodName.contains("Tonight")
        ivDayWeather.getWeatherImage(list[0].weather.toLowerCase(), isNight)
        val high: String = weatherInterface.getTodaysHigh(isNight)
        val low: String = weatherInterface.getTodaysLow(isNight)
        ...
        if (isNight) {
            for (i in 3 until list.size - 4) {
                if (i % 2 == 1) {
                    val miniDay = MiniDayView(context)
                    miniDay.setData(list[i].startPeriodName, list[i].weather.toLowerCase(), list[i].temperature ?: "--", list.getOrNull(i - 1)?.temperature ?: "-", list[i].iconLink)
                    view.llDayConditions.addView(miniDay)
                }
            }
        } else {
        ...
        tvCurrentObservation.text = context.getString(R.string.last_forecast_update, weatherInterface.getCurrentObservationDate())
    }
